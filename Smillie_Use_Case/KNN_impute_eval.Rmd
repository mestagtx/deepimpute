...
seurat_knn_ranks<-function(seurat_obj, k, g, c, indexes) {
  ...
  #prc_n <- as.matrix(seurat_obj@assays$RNA@data)
  #prc_n <- as.data.frame(t(prc_n[g, c]))
  #prc_n <- seurat_obj@assays$RNA@data[g, c] %>% as.matrix(.) %>% t(.) %>% as.data.frame(.)
   prc_n <- as.data.frame(t(as.matrix(seurat_obj@assays$RNA@data[g, c])))

  #prc_n$id <- rownames(seurat_obj@meta.data)[c]
  prc_n$id <- c
  prc_n$celltype <-factor(seurat_obj@meta.data[c,'Cluster'])
  #prc_n$celltype <- factor(seurat_obj@meta.data$Cluster[c])
