...
#raw_data<-read.csv(file="~/CCI/cellphoneDB/smillie_ibd/smillie_fibs_raw_counts.tsv", sep='\t', header = TRUE, row.names = 1) %>% t(.) %>% as("dgCMatrix")
#imputed_data<-read.csv(file="~/CCI/cellphoneDB/smillie_ibd/smillie_fibs_raw_counts_imputed.tsv", sep='\t', header = TRUE, row.names = 1) %>% t() %>% as("dgCMatrix")

meta <- read.csv('~/IBD/Smillie_IBD/all.meta2.txt', sep = "\t", header = TRUE, row.names = 1) 
meta <- meta[-1,]

temp_qc_file=CreateSeuratObject(counts=raw_data, meta.data = meta, min.cells = 3, min.features = 100, names.field = 1)
...
saveRDS("temp_qc_file", file='smillie_temp_qc_file.rds')
```
...
```{r Plot1, echo=FALSE, include=TRUE, fig.height=5, fig.width=10}

print(
  DimPlot(temp_qc_file, reduction = "umap",label=TRUE,group.by='Cluster', seed = 1) 
  )
```
...
Human_M202 <- as.character(Human_CAF_markers$M202[Human_CAF_markers$M202 != ""])
...
```{r Plot1, echo=FALSE, include=TRUE, fig.height=10, fig.width=15}
...
Idents(temp_qc_file) <- "seurat_clusters"
VlnPlot(temp_qc_file, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"),  ncol =3)

DimPlot(temp_qc_file, reduction = "umap", label=T, group.by='seurat_clusters', label.size = 10, pt.size = 2, label.box = TRUE, repel = TRUE) + labs(title = paste0("cluster cells - res-", cluster_res))

DimPlot(temp_qc_file, reduction = "umap",label=TRUE,group.by='Cluster', seed = 1,label.size = 10, pt.size = 2, label.box = TRUE, repel = TRUE) 
...
```{r Plot1, echo=FALSE, include=TRUE, fig.height=10, fig.width=10}
